...
def queue_prompt(prompt):
    ...
    data = json.dumps(p).encode('utf-8')  # Encode the data to JSON format
    req = urllib.request.Request("http://{}/prompt".format(server_address), data=data)
    return json.loads(urllib.request.urlopen(req).read())  # Sending the request and returning the response

# Gets an image from the server based on specified parameters
def get_image(filename, subfolder, folder_type):
    data = {"filename": filename, "subfolder": subfolder, "type": folder_type}
    url_values = urllib.parse.urlencode(data)  # Encoding the parameters into a URL query string
    with urllib.request.urlopen("http://{}/view?{}".format(server_address, url_values)) as response:
        return response.read()  # Making the request and returning the response (image data)

# Gets the history of a prompt's execution based on its ID
def get_history(prompt_id):
    with urllib.request.urlopen("http://{}/history/{}".format(server_address, prompt_id)) as response:
        return json.loads(response.read())  # Returning the execution history
...
for id in prompt_workflow:
    ...
    if node["_meta"]["title"] == 'CLIP Text Encode (Pos Prompt)':
        prompt_pos_node = node
        break  # Exit the loop once the desired node is found
